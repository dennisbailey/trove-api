...
router.get('/messages', function(req, res, next) {
    ...
    .then(function(result) {
        res.status(200).json({
            status: 'All the great Messages for Market ID ' + req.query.id,
            data: result
        });
    })
    ...
    .catch(function(error) {
        return res.status(401)
            .json({
                status: 'There was an error getting messages for this market',
                errorMsg: error
            });
    });
...
router.post('/messages', function(req, res, next) {
    ...
    .then(function(result) {
        global.io.emit('message.new', req.body.market_id);
        res.status(200)
            .json({
                status: 'Another message successfully saved. For posterity.',
                data: result
            });
    })
    ...
    .catch(function(error) {
        return res.status(401)
            .json({
                status: 'There was an error posting this message.',
                errorMsg: error
            });
    });
...
router.get('/images', function(req, res, next) {
    ...
    .then(function(result) {
        res.status(200).json({
            status: 'All the great Images for Market ID ' + req.query.id,
            data: result
        });
    })
    ...
    .catch(function(error) {
        return res.status(401)
            .json({
                status: 'There was an error getting the images for this market',
                errorMsg: error
            });
    });
            ...
            if (err) {
                ...
                s3.putObject(s3_param, function(err, data) {
                    ...
                    if (err) {
                        ...
                        .then(function(result) {
                            global.io.emit('image.new', req.body.marketID);
                        })
...
router.get('/vendors', function(req, res, next) {
    ...
    .then(function(result) {
        return res.status(200)
            .json({
                status: 'All the great vendors for this Farmers Market',
                vendors: result
            });
    })
    ...
    .catch(function(error) {
        return res.status(401)
            .json({
                status: 'There was an error getting vendors for this market',
                errorMsg: error
            });
    });
